...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 15, 2023 at 08:36 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_update_entry_by_user_id_and_entry_id` (IN `userId` INT, IN `entryId` INT, IN `entryNotes` TEXT, IN `moodName` VARCHAR(255), IN `activityCommaDelimStr` VARCHAR(1000))   BEGIN
    ...
    -- SELECT activityName FROM (CALL usp_select_activities_by_user_id_and_activity_names(userId, activityCommaDelimStr));
END$$
...
(50, 'MMMMMMMMMMMMMMMMMNNNNNNNNNNNNNNNNN', '2023-01-14 00:06:29', 94, 70),
(51, 'vbvbvbvbvbvbvb', '2023-01-14 00:07:37', 94, 67),
...
--
ALTER TABLE `tbl_entry_activity`
  MODIFY `entry_activity_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=113;
