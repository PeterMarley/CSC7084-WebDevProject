...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 24, 2022 at 12:19 PM
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_Check_Password_Login` (`username` VARCHAR(255), `plainPassword` VARCHAR(255)) RETURNS VARBINARY(255)  BEGIN
    ...
    RETURN IF(fn_1WayEncrypt(plainPassword, salt) = storedPassword, storedPassword, 0);
END$$
